-- =====================================================
-- COMPREHENSIVE COMPLIANCE FIX - FIXED VERSION
-- =====================================================
...
-- =====================================================
-- STEP 7: CREATE NECESSARY FUNCTIONS (WITH PROPER HANDLING)
-- =====================================================

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS public.get_ca_startups(VARCHAR(50));
DROP FUNCTION IF EXISTS public.assign_ca_to_startup(VARCHAR(50), INTEGER, TEXT);
DROP FUNCTION IF EXISTS public.remove_ca_assignment(VARCHAR(50), INTEGER);
DROP FUNCTION IF EXISTS public.get_ca_assignment_requests(VARCHAR(50));
DROP FUNCTION IF EXISTS public.approve_ca_assignment_request(INTEGER, VARCHAR(50));
DROP FUNCTION IF EXISTS public.reject_ca_assignment_request(INTEGER, VARCHAR(50), TEXT);
...
DROP POLICY IF EXISTS "CAs can view their own assignments" ON public.ca_assignments;
DROP POLICY IF EXISTS "CAs can create their own assignments" ON public.ca_assignments;
DROP POLICY IF EXISTS "CAs can update their own assignments" ON public.ca_assignments;
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "CAs can view requests for them" ON public.ca_assignment_requests;
DROP POLICY IF EXISTS "CAs can update requests for them" ON public.ca_assignment_requests;
