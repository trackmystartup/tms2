-- FIX_FORM2_OPTIONAL_CA_LICENSE.sql
-- This script fixes Form 2 completion with optional ca_license
...
-- Step 2: Fix Form 2 for Startup users (need government_id + startup data, ca_license optional)
UPDATE public.users 
SET 
...
-- Step 3: Fix Form 2 for Investor users (need government_id only, ca_license optional)
UPDATE public.users 
SET 
  government_id = COALESCE(government_id, 'https://placeholder-document-url.com/government-id'),
  ca_license = COALESCE(ca_license, 'https://placeholder-document-url.com/company-registration'),
...
-- Step 4: Fix Form 2 for Investment Advisor users (need government_id + license, ca_license optional)
UPDATE public.users 
SET 
  government_id = COALESCE(government_id, 'https://placeholder-document-url.com/government-id'),
  ca_license = COALESCE(ca_license, 'https://placeholder-document-url.com/company-registration'),
...
-- Step 5: Fix Form 2 for Startup Facilitation Center users (need government_id only, ca_license optional)
UPDATE public.users 
SET 
  government_id = COALESCE(government_id, 'https://placeholder-document-url.com/government-id'),
  ca_license = COALESCE(ca_license, 'https://placeholder-document-url.com/company-registration'),
...
-- Step 7: Verify Form 2 completion by role (government_id is the only mandatory field)
SELECT 'Form 2 completion status by role (government_id mandatory, ca_license optional):' as info;
SELECT 
  u.role,
