-- RAZORPAY INTEGRATION SQL
-- Run this in Supabase SQL editor to enable real Razorpay integration

-- 1. Drop existing function if it exists and create new one
DROP FUNCTION IF EXISTS create_razorpay_order(UUID, DECIMAL, TEXT, TEXT);
DROP FUNCTION IF EXISTS create_razorpay_order(UUID, DECIMAL, TEXT);
...
-- 2. Drop existing function if it exists and create new one
DROP FUNCTION IF EXISTS verify_razorpay_payment(TEXT, TEXT, TEXT);
...
-- 3. Drop existing function if it exists and create new one
DROP FUNCTION IF EXISTS handle_razorpay_webhook(TEXT, JSON);
...
-- 4. Create RLS policies for new functions
