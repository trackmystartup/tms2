-- Clean up duplicates and create the automatic system
-- This will remove duplicate records and then create the automatic system
...
SELECT 
    'Duplicate Offers Check' as info,
    startup_id,
    investor_email,
    COUNT(*) as duplicate_count
FROM investment_offers
GROUP BY startup_id, investor_email
HAVING COUNT(*) > 1
ORDER BY duplicate_count DESC;

-- Check for duplicate relationships
SELECT 
    'Duplicate Relationships Check' as info,
    investment_advisor_id,
    startup_id,
    relationship_type,
    COUNT(*) as duplicate_count
FROM investment_advisor_relationships
GROUP BY investment_advisor_id, startup_id, relationship_type
HAVING COUNT(*) > 1
ORDER BY duplicate_count DESC;

-- =====================================================
-- 2. CLEAN UP DUPLICATE OFFERS
-- =====================================================

-- Remove duplicate offers, keeping only the most recent one
DELETE FROM investment_offers 
WHERE id NOT IN (
    SELECT DISTINCT ON (startup_id, investor_email) id
    FROM investment_offers
    ORDER BY startup_id, investor_email, created_at DESC
);

-- =====================================================
-- 3. CLEAN UP DUPLICATE RELATIONSHIPS
-- =====================================================

-- Remove duplicate relationships, keeping only the most recent one
DELETE FROM investment_advisor_relationships 
WHERE id NOT IN (
    SELECT DISTINCT ON (investment_advisor_id, startup_id, relationship_type) id
    FROM investment_advisor_relationships
    ORDER BY investment_advisor_id, startup_id, relationship_type, created_at DESC
);

-- =====================================================
-- 4. ADD UNIQUE CONSTRAINTS (NOW SAFE)
-- =====================================================
...
-- Add unique constraint for investment_advisor_relationships table
...
-- =====================================================
-- 5. CREATE AUTOMATIC RELATIONSHIP SYSTEM
-- =====================================================
...
-- =====================================================
-- 6. CREATE TRIGGER FOR AUTOMATIC OFFER CREATION
-- =====================================================
