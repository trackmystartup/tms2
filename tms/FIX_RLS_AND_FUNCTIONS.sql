-- Fix RLS policies and create SECURITY DEFINER functions
-- This will allow the functions to bypass RLS policies

-- =====================================================
-- 1. FIX RLS POLICIES FOR RELATIONSHIPS TABLE
-- =====================================================
...
CREATE POLICY "Allow admin to manage all relationships" ON investment_advisor_relationships
...
CREATE POLICY "Allow users to view their own relationships" ON investment_advisor_relationships
...
-- =====================================================
-- 2. FIX RLS POLICIES FOR OFFERS TABLE
-- =====================================================
...
-- Create new policies
CREATE POLICY "Allow admin to manage all offers" ON investment_offers
FOR ALL TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM users 
...
CREATE POLICY "Allow users to view offers for their startups" ON investment_offers
FOR SELECT TO authenticated
USING (
  startup_id IN (
    SELECT id FROM startups WHERE user_id = auth.uid()
...
-- =====================================================
-- 3. RECREATE FUNCTIONS WITH SECURITY DEFINER
-- =====================================================

-- Function to create relationships for existing data (SECURITY DEFINER)
...
-- Function to create offers for existing relationships (SECURITY DEFINER)
...
-- =====================================================
-- 4. UPDATE AUTOMATIC TRIGGERS WITH SECURITY DEFINER
-- =====================================================

-- Function to automatically create advisor-startup relationships (SECURITY DEFINER)
...
-- Function to automatically create investment offers (SECURITY DEFINER)
