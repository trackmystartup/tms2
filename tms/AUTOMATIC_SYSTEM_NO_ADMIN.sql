-- AUTOMATIC SYSTEM WITHOUT ADMIN INTERVENTION
-- This creates a system that works for all users automatically

-- =====================================================
-- 1. FIX RLS POLICIES FOR ALL USERS
-- =====================================================

-- Drop existing restrictive policies
DROP POLICY IF EXISTS "Users can view their own advisor relationships" ON investment_advisor_relationships;
DROP POLICY IF EXISTS "Users can insert their own advisor relationships" ON investment_advisor_relationships;
DROP POLICY IF EXISTS "Users can update their own advisor relationships" ON investment_advisor_relationships;
DROP POLICY IF EXISTS "Allow admin to manage all relationships" ON investment_advisor_relationships;

DROP POLICY IF EXISTS "Users can view their own offers" ON investment_offers;
DROP POLICY IF EXISTS "Users can insert their own offers" ON investment_offers;
DROP POLICY IF EXISTS "Users can update their own offers" ON investment_offers;
DROP POLICY IF EXISTS "Allow admin to manage all offers" ON investment_offers;

-- Create open policies for automatic system
CREATE POLICY "Allow all authenticated users to manage relationships" ON investment_advisor_relationships
FOR ALL TO authenticated
USING (true);

CREATE POLICY "Allow all authenticated users to manage offers" ON investment_offers
FOR ALL TO authenticated
USING (true);

-- =====================================================
-- 2. CREATE AUTOMATIC TRIGGERS (NO ADMIN NEEDED)
-- =====================================================
...
DROP TRIGGER IF EXISTS trg_create_advisor_relationship ON startups;
DROP TRIGGER IF EXISTS trg_create_investment_offers ON investment_advisor_relationships;

-- Create trigger for startup advisor code updates
CREATE TRIGGER trg_create_advisor_relationship
  AFTER UPDATE OF investment_advisor_code ON startups
  FOR EACH ROW
  EXECUTE FUNCTION create_advisor_relationships_automatically();

-- Create trigger for new relationships
CREATE TRIGGER trg_create_investment_offers
  AFTER INSERT ON investment_advisor_relationships
  FOR EACH ROW
  EXECUTE FUNCTION create_investment_offers_automatically();

-- =====================================================
-- 4. CREATE USER-FRIENDLY FUNCTIONS (NO ADMIN NEEDED)
-- =====================================================

-- Function to create relationships for existing data (works for any user)
...
-- Function to create offers for existing relationships (works for any user)
