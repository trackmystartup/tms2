-- Verification Script for Startup Subscription Plans (FIXED)
-- Run this after executing STARTUP_SUBSCRIPTION_PLANS_SCHEMA_FIXED.sql
...
AND column_name IN ('is_in_trial', 'trial_start', 'trial_end', 'razorpay_subscription_id', 'billing_interval')
...
AND (indexname LIKE '%trial%' OR indexname LIKE '%startup%' OR indexname LIKE '%billing%')
...
DECLARE
    ...
    views_count INTEGER;
    billing_interval_column_exists BOOLEAN;
BEGIN
    -- Check plans
    ...
    -- Check billing_interval column specifically
    SELECT EXISTS(
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_subscriptions' 
        AND column_name = 'billing_interval'
    ) INTO billing_interval_column_exists;
    ...
    RAISE NOTICE 'Billing Interval Column: %', CASE WHEN billing_interval_column_exists THEN 'EXISTS' ELSE 'MISSING' END;
    RAISE NOTICE 'Views Created: %', views_count;
    
    IF plan_count >= 2 AND trial_columns_count = 4 AND billing_interval_column_exists AND views_count >= 2 THEN
    ...
    ELSE
        ...
        RAISE WARNING 'Expected: 2+ plans, 4 trial columns, billing_interval column, 2+ views';
        RAISE WARNING 'Found: % plans, % trial columns, billing_interval: %, % views', 
            plan_count, trial_columns_count, billing_interval_column_exists, views_count;
    END IF;
END $$;
