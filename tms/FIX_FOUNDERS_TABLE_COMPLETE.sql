-- =====================================================
-- COMPLETE FOUNDERS TABLE FIX
-- =====================================================
-- This script fixes all founders table issues in the correct order
...
SELECT 
    'Founders table structure after complete fix:' as info,
    column_name,
    data_type,
    is_nullable,
    column_default
...
-- Step 6: Test the trigger by updating a record
DO $$
DECLARE
    test_startup_id INTEGER;
    test_founder_id TEXT;
    founder_count INTEGER;
BEGIN
    -- Check if there are any founders
    SELECT COUNT(*) INTO founder_count FROM public.founders;
    
    IF founder_count > 0 THEN
        -- Get a startup and founder for testing
        SELECT f.startup_id::TEXT, f.id::TEXT INTO test_startup_id, test_founder_id
        FROM public.founders f
        LIMIT 1;
        
        IF test_founder_id IS NOT NULL THEN
            -- Test the trigger by updating the founder
            UPDATE public.founders 
            SET name = name || ' (test)'
            WHERE id::TEXT = test_founder_id;
            
            -- Revert the test change
            UPDATE public.founders 
            SET name = REPLACE(name, ' (test)', '')
            WHERE id::TEXT = test_founder_id;
            
            RAISE NOTICE '✅ Trigger test successful - founders table is working correctly';
        ELSE
            RAISE NOTICE 'No valid founder ID found for trigger testing';
        END IF;
    ELSE
        RAISE NOTICE 'No founders found for trigger testing';
    END IF;
END $$;
